...
# from .queries import MissingFieldsQueries
# from .sheets import MissingNmSheet, MissingChrtSheet, VatMismatchSheet
...
#     def generate_complete_package(self, upd_ids: List[int] = None) -> BytesIO:
#         """Один метод - один запрос к БД - все отчеты"""
        
#         data = self.queries.get_stats_and_reports(upd_ids)
#         stats = data['stats']
...
#             # NM_ID отчет
#             if not data['nm_df'].empty:
#                 wb = self._create_workbook()
#                 MissingNmSheet(wb, data['nm_df'], stats).build()
#                 buffer = BytesIO()
#                 wb.save(buffer)
#                 buffer.seek(0)
#                 zip_file.writestr("Отчет_товары_без_NM_ID.xlsx", buffer.getvalue())
            
#             # CHRT_ID отчет
#             if not data['chrt_df'].empty:
#                 wb = self._create_workbook()
#                 MissingChrtSheet(wb, data['chrt_df'], stats).build()
#                 buffer = BytesIO()
#                 wb.save(buffer)
#                 buffer.seek(0)
#                 zip_file.writestr("Отчет_товары_без_CHRT_ID.xlsx", buffer.getvalue())
            
#             # НДС отчет
#             if not data['vat_df'].empty:
#                 wb = self._create_workbook()
#                 VatMismatchSheet(wb, data['vat_df'], stats).build()
#                 buffer = BytesIO()
#                 wb.save(buffer)
#                 buffer.seek(0)
#                 zip_file.writestr("Отчет_несовпадение_НДС.xlsx", buffer.getvalue())
        
#         zip_buffer.seek(0)
#         return zip_buffer
    
#     def get_report_response(self, upd_ids: List[int] = None) -> HttpResponse:
#         from datetime import datetime
#         timestamp = datetime.now().strftime("%Y%m%d_%H%M")
#         buffer = self.generate_complete_package(upd_ids)
        
#         response = HttpResponse(buffer.getvalue(), content_type="application/zip")
#         response["Content-Disposition"] = f'attachment; filename="reports_{timestamp}.zip"'
#         return response
...
class MissingFieldsReportGenerator:
    ...
    def generate_complete_package(self, upd_ids: List[int] = None) -> BytesIO:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            # PDF (без детализации по УПД, только общая статистика)
            pdf_response = build_missing_fields_pdf_response(stats)
            zip_file.writestr("Сопроводительное_письмо.pdf", pdf_response.content)
            
            # NM_ID отчет (всегда создаем)
            wb = self._create_workbook()
            MissingNmSheet(wb, data['nm_df'], stats).build()
            buffer = BytesIO()
            wb.save(buffer)
