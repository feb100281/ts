# cards/reporting/reconcile/sheets/duplicates_sheet.py
...
class DuplicatesSheet:
    def __init__(self, workbook, sheet_number):
        self.wb = workbook
        self.sheet_number = sheet_number
        sheet_name = f"{sheet_number:02d}_Дубликаты"
    ...
    def build(self, df):
        ...
        row = self.title.draw(
            row=row,
            title="ДУБЛИКАТЫ В СИСТЕМЕ",
            subtitle="УПД с одинаковыми контрагентом, датой и номером",
        ...
        if len(df) == 0:
            ...
            cell = self.ws.cell(row=row, column=2, value="✅ Нет дубликатов в системе")
        ...
        # Подсветка дубликатов - группируем по номеру и дате
        if len(df) > 0:
            # Находим дублирующиеся группы
            seen = {}
            for idx, (_, row_data) in enumerate(df.iterrows()):
                key = f"{row_data.get('number_our', '')}_{row_data.get('date', '')}"
                if key not in seen:
                    seen[key] = []
                seen[key].append(idx)
            
            # Подсвечиваем строки-дубликаты
            row_idx = row
            for _, row_data in df.iterrows():
                key = f"{row_data.get('number_our', '')}_{row_data.get('date', '')}"
                if len(seen[key]) > 1:
                    # Это дубликат, подсвечиваем всю строку
                    for col in range(2, 7):  # Колонки B-F
                        cell = self.ws.cell(row=row_idx, column=col)
                        cell.fill = FILLS.get("warning", PatternFill(fill_type=None))
                        if col == 6:  # Колонка суммы
                            cell.font = Font(name="Roboto", size=10, bold=True, color=COLORS["warning_text"])
                row_idx += 1
...
def create_duplicates_sheet(workbook, sheet_number, df):
    """Создает лист с дубликатами"""
    sheet = DuplicatesSheet(workbook, sheet_number)
    sheet.build(df)
    return sheet.ws