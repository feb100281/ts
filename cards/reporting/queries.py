...
#     def get_vat_mismatch_data(self, upd_ids: List[int] = None) -> pd.DataFrame:
#         """Строки, где ставка НДС в УПД не совпадает со ставкой в карточке товара"""
...
#                 t.upd_vat_rate AS upd_vat_rate,
#                 p.vat_rate AS card_vat_rate,
...
#                 AND t.upd_vat_rate IS NOT NULL
#                 AND t.upd_vat_rate > 0
#                 AND p.vat_rate IS NOT NULL
#                 AND t.upd_vat_rate != p.vat_rate
#                 {upd_condition}
...
#     def get_grouped_vat_mismatch(self, upd_ids: List[int] = None) -> pd.DataFrame:
#         """Сгруппированный отчет по товарам с несовпадением НДС"""
        
#         df = self.get_vat_mismatch_data(upd_ids)
...
#     def get_vat_mismatch_by_upd(self, upd_ids: List[int] = None) -> List[Dict]:
#         """Получить распределение проблем с НДС по УПД"""
...
#                 AND t.upd_vat_rate > 0
#                 AND EXISTS (
#                     SELECT 1 FROM analytics.cards.product p
#                     WHERE p.nm_id = t.nm_id 
#                     AND p.vat_rate IS NOT NULL
#                     AND t.upd_vat_rate != p.vat_rate
#                 )
#                 AND t.upd_document_id IN ({placeholders})
...
class MissingFieldsQueries:
    """Запросы для отчетов по проблемам в УПД"""
    ...
    def _get_data(self, upd_ids: List[int] = None) -> pd.DataFrame:
    ...
    def get_stats_and_reports(self, upd_ids: List[int] = None) -> Dict:
        """Возвращает статистику и все три отчета одним махом"""
        
        df = self._get_data(upd_ids)
        
        if df.empty:
            return {
                'stats': self._empty_stats(),
                'nm_df': pd.DataFrame(),
                'chrt_df': pd.DataFrame(),
                'vat_df': pd.DataFrame(),
            }
        
        # Статистика
        total_amount = df['upd_amount_vatadd'].sum() or 0
        
        nm_mask = df['nm_id'].isna()
        chrt_mask = df['chrt_id'].isna()
        vat_mask = (
            df['nm_id'].notna() & 
            df['upd_vat_rate'].notna() & 
            (df['upd_vat_rate'] > 0) &
            df['card_vat_rate'].notna() &
            (df['upd_vat_rate'] != df['card_vat_rate'])
        )
        
        stats = {
            'total_upd_count': len(df['upd_document_id'].unique()),
            'total_lines': len(df),
            'total_amount_vatadd': total_amount,
            
            'missing_nm_count': nm_mask.sum(),
            'missing_nm_qty': df[nm_mask]['upd_qty'].sum() or 0,
            'missing_nm_amount': df[nm_mask]['upd_amount_vatadd'].sum() or 0,
            
            'missing_chrt_count': chrt_mask.sum(),
            'missing_chrt_qty': df[chrt_mask]['upd_qty'].sum() or 0,
            'missing_chrt_amount': df[chrt_mask]['upd_amount_vatadd'].sum() or 0,
            
            'vat_mismatch_count': vat_mask.sum(),
            'vat_mismatch_qty': df[vat_mask]['upd_qty'].sum() or 0,
            'vat_mismatch_amount': df[vat_mask]['upd_amount_vatadd'].sum() or 0,
        }
        
        # Отчеты
        nm_df = self._group_nm(df[nm_mask]) if nm_mask.any() else pd.DataFrame()
        chrt_df = self._group_chrt(df[chrt_mask]) if chrt_mask.any() else pd.DataFrame()
        vat_df = self._group_vat(df[vat_mask]) if vat_mask.any() else pd.DataFrame()
    ...
    def _group_nm(self, df: pd.DataFrame) -> pd.DataFrame:
        if df.empty:
            return df
        
        group_cols = ['upd_sa_name', 'brand', 'upd_title', 'upd_size']
        return self._group(df, group_cols)
    
    def _group_chrt(self, df: pd.DataFrame) -> pd.DataFrame:
        if df.empty:
            return df
        
        group_cols = ['upd_sa_name', 'brand', 'upd_title', 'upd_size', 'nm_id', 'wb_sa_name', 'available_sizes']
        return self._group(df, group_cols)
    
    def _group_vat(self, df: pd.DataFrame) -> pd.DataFrame:
        if df.empty:
            return df
        
        group_cols = ['upd_sa_name', 'brand', 'upd_title', 'upd_size', 'nm_id', 'wb_sa_name', 'upd_vat_rate', 'card_vat_rate', "upd_price_vatadd",]
        return self._group(df, group_cols)
    
    def _group(self, df: pd.DataFrame, group_cols: List[str]) -> pd.DataFrame:
        grouped = (
            df.groupby(group_cols, dropna=False)
            .agg(
                upd_info=('upd_info', lambda x: ', '.join(sorted(set(str(v) for v in x if pd.notna(v))))),
                counterparty_name=('counterparty_name', lambda x: ', '.join(sorted(set(str(v) for v in x if pd.notna(v))))),
                upd_qty=('upd_qty', 'sum'),
                upd_amount_vatadd=('upd_amount_vatadd', 'sum'),
            )
            .reset_index()
        ...
        grouped['upd_price_vatadd'] = grouped.apply(
            lambda row: row['upd_amount_vatadd'] / row['upd_qty'] if row['upd_qty'] else None,
            axis=1
        )
        
        return grouped.sort_values('upd_amount_vatadd', ascending=False)
    
    def _empty_stats(self) -> Dict:
        return {
            'total_upd_count': 0, 'total_lines': 0, 'total_amount_vatadd': 0,
            'missing_nm_count': 0, 'missing_nm_qty': 0, 'missing_nm_amount': 0,
            'missing_chrt_count': 0, 'missing_chrt_qty': 0, 'missing_chrt_amount': 0,
            'vat_mismatch_count': 0, 'vat_mismatch_qty': 0, 'vat_mismatch_amount': 0,
        }