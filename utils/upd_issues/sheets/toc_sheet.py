...
#     def build(self):
#         """Создает страницу справки для конкретного УПД"""
#         row = 1
#         table_start_col = 2  # Колонка B
#         table_end_col = 5     # Колонка E (включительно)
...
#         # Создаем светло-серую заливку для блока
...
#         # БЛОК НАВИГАЦИИ
#         # ============================================================
#         nav_cell = self.ws.cell(row=row, column=table_start_col, value="ЛИСТЫ С НЕСООТВЕТСТВИЯМИ")
...
#             sheets.append(("01_Названия", "Несоответствие названий", self.stats['name_mismatch']))
#         if self.stats.get('size_mismatch', 0) > 0:
#             sheets.append(("02_Размеры", "Несоответствие размеров", self.stats['size_mismatch']))
#         if self.stats.get('vat_mismatch', 0) > 0:
#             sheets.append(("03_НДС", "Несоответствие ставки НДС", self.stats['vat_mismatch']))
#         if self.stats.get('cert_issues', 0) > 0:
#             sheets.append(("04_Сертификаты", "Проблемы с сертификатами", self.stats['cert_issues']))
...
#         self.ws.column_dimensions["C"].width = 38  # Тип ошибки
#         self.ws.column_dimensions["D"].width = 14  # Количество
#         self.ws.column_dimensions["E"].width = 10  # Перейти
