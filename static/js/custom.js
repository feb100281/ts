  ...
  function ensureModal() {
    ...
    backdrop.innerHTML = `
      <div class="jm-manpack-modal" role="dialog" aria-modal="true" aria-labelledby="jmMp2Title">
        ...
        <label class="jm-manpack-label" for="jmMp2Date">Выгрузки в csv, разделитель &laquo;|&raquo;</label>
        <div class="jm-manpack-quick">
          <button type="button" class="jm-manpack-quick-btn" id="jmMp2CsvCf">
            Cash Flow, csv
          </button>
          <button type="button" class="jm-manpack-quick-btn" id="jmMp2CsvPl">
            P&amp;L, csv
          </button>
        </div>
    ...
    function download(baseUrl) {
      ...
      errorBox.classList.remove("is-visible");
      close();
      window.location.href = `${baseUrl}?report_date=${encodeURIComponent(reportDate)}`;
    }
    ...
    backdrop.querySelector("#jmMp2CsvPl").addEventListener("click", function () {
      download(PL_CSV_URL);
    });
