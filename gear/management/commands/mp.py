...
NOTES_PL = [
    ("Назначение отчёта", [
        ...
        "Единица измерения — рубли. Расходы показаны со знаком минус.",
    ]),
    ("Признание выручки: почему продажи не равны деньгам", [
        "Выручка признаётся по отчёту маркетплейса, но только по тем единицам "
        "товара, которые удалось сопоставить с себестоимостью. Единицы без "
        "сопоставления в выручку периода не попадают — их количество показано "
        "справочно в строке 2.3 «Без себестоимости, шт.».",
    ...
    ("Себестоимость: FIFO и пересчёт задним числом", [
        ...
        "Практический вывод: последние два-три месяца отчёта считаются "
    ...
    ("Как читать колонки", [
        ...
        "месяца). Ряд месяцев всегда начинается с января 2025 года.",
...
NOTES_CF = [
    ("Назначение отчёта", [
        ...
        "баланса WB. Сумма строки = дебет минус кредит: поступление положительно, "
        "платёж отрицателен.",
    ]),
    ...
    ("200000 Инвестиционная деятельность", [
        "Приобретение основных средств и вложения в инфраструктуру. В текущем периметре "
        "блок минимален, но выделен отдельно для сопоставимости с будущими периодами.",
    ]),
...
def build_cover(ws, report_date, months, kpi, out_name, contents=None, as_of=None):
    ...
    r = 7
    cards = [
        ("ВЫРУЧКА БЕЗ НДС", kpi.get("revenue"), FMT_MONEY, True),
    ...
    # ---- содержание
    r += 5
    write(ws, r, 2, "СОДЕРЖАНИЕ ПАКЕТА", font=Font(name=FONT, size=11, bold=True,
                                                   color=TEXT),
    ...
    if contents is None:
        ...
        ("Пояснения Расшифровки", "Как устроены ноты и как они сверяются с P&L"),
            ("Пояснения Cash Flow", "Структура потоков, остатки и связь с P&L"),
            ("Источники данных", "Витрины, запросы и регламент обновления пакета"),
        ]
...
NOTES_UNIT = [
    ("Что показывает лист", [
        ...
        "Состав показателей согласован с финансовой службой. Все строки, "
        "которые выводятся из других строк, записаны формулами Excel: щёлкните "
        "на ячейке — и увидите, из чего получено число. Макросов в файле нет.",
    ]),
    ...
    ("Раздел 1. Объёмы и выручка", [
        ...
        "продаже вычтен НДС по её фактической ставке (10 % или 20 %, известна "
    ...
    ("Раздел 3. Себестоимость и удержания площадки", [
        "Все строки раздела показаны со знаком минус (в скобках) и БЕЗ НДС — "
        "так же, как в формулах маржинальности, согласованных с финансовой "
        "службой.",
        ...
        "ВАЖНО: в P&L те же статьи (разделы 3 и 4) показаны С НДС — там это "
        "сумма фактического удержания площадки. Здесь они приведены без НДС, "
        "потому что сопоставляются с выручкой без НДС. Разница между листами "
        "по этим строкам — сумма входящего НДС, это ожидаемо.",
    ]),
    ...
    ("Как читать колонки", [
        ...
        "годовых сумм — среднее арифметическое по месяцам там было бы "
        "некорректным.",
    ]),
]
...
def build_treasury(ws, bank_rows, wb_row, report_date, as_of=None, deposit_balance=0.0):
    ...
    wb_lines = [
        ("К перечислению продавцу, всего",
         _f(wb_row[4]) if wb_row else 0.0, f_income, False),
        ("Выведено средств, всего",
         _f(wb_row[5]) if wb_row else 0.0, f_expense, False),
        ("Конечный баланс без денег в пути",
         _f(wb_row[8]) if wb_row else 0.0, f_label, False),
        ("Деньги в пути (ДВП)", transit, f_label, False),
        ("Конечный баланс WB", wb_balance, f_bold_num, True),
    ]
    for lbl, v, fnt, is_last in wb_lines:
...
def build_pl_notes(ws, note_rows, months, report_date, as_of=None):
    ...
    def collapse(node):
        """Свернуть уровень, если он пустой («» — значит данных нет)."""
        return [(k, v) for k, v in node.items() if k]
    ...
    for section in sorted(tree):
        ...
        for item, lvl1_node in tree[section].items():
            ...
            for lvl1, lvl2_node in lvl1_node.items():
                ...
                for lvl2, lvl3_node in collapse(lvl2_node):
                    ...
                    if r > first_lvl3:
                        sum_row(lvl2_row, lvl2, first_lvl3, r - 1,
                                font=f_lvl_deep, num_font=f_num, bg=None,
                                bd=B_BOTTOM, indent=5, outline=2)
                    else:
                        money_row(lvl2_row, lvl2, lvl3_node.get("", {}),
                                  font=f_lvl_deep, num_font=f_num, bg=None,
                                  bd=B_BOTTOM, indent=5, outline=2)

                if r > first_lvl2:
                    sum_row(lvl1_row, lvl1 or "Без детализации",
                            first_lvl2, r - 1, font=f_lvl, num_font=f_num,
                            bg=None, bd=B_BOTTOM, indent=3, outline=1)
                else:
                    vals = lvl2_node.get("", {}).get("", {})
                    money_row(lvl1_row, lvl1 or "Без детализации", vals,
                              font=f_lvl, num_font=f_num, bg=None,
                              bd=B_BOTTOM, indent=3, outline=1)

            sum_row(item_row, item, first_child, r - 1, font=f_item,
        ...
        for i in range(len(cols)):
            cl = L(i)
            v = ("=" + "+".join("%s%d" % (cl, rr) for rr in item_rows)
                 if item_rows else 0)
...
NOTES_PL_SHEET = [
    ("Что это за лист", [
        "Расшифровка каждой строки разделов 3-8 отчёта о прибылях и убытках: "
        "из чего сложилась сумма и кому она заплачена.",
        "Переход сюда — из P&L: в колонке «НОТА» напротив строки стоит "
        "стрелка-ссылка, она ведёт ровно на нужный блок этого листа. "
        "Обратно — кнопка «Оглавление» в левом верхнем углу.",
    ]),
    ("Три уровня детализации", [
        "Первый уровень — статья затрат внутри группы, которая показана в "
        "P&L. Второй — контрагент. Третий — договор или основание платежа.",
        "Лист открывается свёрнутым до статьи: видно, из чего состоит группа. "
        "Контрагенты и договоры раскрываются кнопками «+» слева от таблицы.",
        "Для удержаний маркетплейса (разделы 3 и 4) контрагент всегда один — "
        "сама площадка, поэтому там детализация заканчивается на статье "
        "удержания.",
    ]),
    ("Сверка с P&L", [
        "Итог раздела на этом листе равен строке «Итого …» соответствующего "
        "раздела в P&L, а сумма статьи — строке статьи. Обе цифры считаются "
        "из одной витрины, поэтому расхождений быть не должно.",
        "Если сумма всё же не сходится, причина одна из двух: в P&L статья "
        "попала в другой раздел (проверьте счёт учёта) или проводка сделана "
        "после формирования пакета — сформируйте отчёт заново.",
        "Все суммы показаны в рублях, расходы — в скобках, со знаком минус, "
        "как и в самом P&L.",
    ]),
]
...
# =============================================================================
#  07C. СВОДНЫЕ ТАБЛИЦЫ (нативные пивоты Excel)
#
#      Пакет собирается поверх готового скелета assets/pivot_skeleton.xlsx:
#      в нём лежит лист-источник raw_cf с «умной таблицей» и лист со
#      сводной, построенной на этой таблице.
#      Команда только наполняет источники свежими данными и раздвигает
#      диапазоны таблиц — сводные пересчитываются самим Excel при открытии
#      файла (refreshOnLoad). Никаких макросов и VBA не используется.
# =============================================================================

SKELETON_PATH = Path(__file__).resolve().parent / "assets" / "pivot_skeleton.xlsx"

# Сводная по P&L из пакета убрана: она строилась на бухгалтерской витрине
# и не сходилась с управленческим P&L. Вместо неё — лист «Расшифровки P&L»
# со ссылками из колонки «НОТА».
PIVOT_SHEETS = {
    "CF": ("Сводная CF", "СВОДНАЯ: ДВИЖЕНИЕ ДЕНЕЖНЫХ СРЕДСТВ"),
}
...
def fetch_raw_tables(con, date_from):
    """Сырые витрины под сводные: {имя таблицы: (колонки, строки)}."""
    out = {}
    for name, sql in RAW_TABLE_SQL.items():
        cur = con.execute(sql, parameters={"date_from": date_from})
        cols = [d[0] for d in cur.description]
        out[name] = (cols, cur.fetchall())
    return out


def _cell_value(v):
    """Приводит значение из БД к типу, который принимает openpyxl."""
...
def fill_raw_table(wb, sheet_name, table_name, columns, rows):
    ...
    for i, row in enumerate(rows, start=2):
        for j, v in enumerate(row, start=1):
            ws.cell(row=i, column=j, value=_cell_value(v))

    last_row = max(2, 1 + len(rows))
    ref = "A1:%s%d" % (get_column_letter(len(columns)), last_row)
    ...
    for pivots in (getattr(w, "_pivots", []) for w in wb.worksheets):
        for pt in pivots:
...
def style_pivot_sheet(ws, title):
    ...
    c.value = "\u2190  Оглавление"
    c.hyperlink = "#'%s'!A1" % TOC_SHEET_NAME
    c.font = Font(name=FONT, size=9, bold=True, color=NAVY_3)
    c.alignment = Alignment(horizontal="left", vertical="center", indent=1)
    c.fill = fill(SURFACE_4)
    c.border = Border(bottom=_side(LINE))
    ...
    if ws.cell(row=1, column=2).value is None:
        head.value = title
    head.font = Font(name=FONT, size=12, bold=True, color=TEXT)
...
def notes_sources(date_from, start_year, months):
    return [
        ("Команда и параметры", [
            "python manage.py mp <date_from> [--start-year YYYY] [--out PATH]",
            "date_from — верхняя граница данных; отчётным месяцем считается "
            "последний день этого месяца. Колонки всегда начинаются с января "
            "--start-year (по умолчанию %d) и идут до отчётного месяца. "
            "Текущий запуск: %s, период с %d года, месяцев в колонках: %d."
            % (DEFAULT_START_YEAR, date_from.isoformat(), start_year, len(months)),
            "Тот же пакет можно скачать из админки: меню «Экспорт» → "
            "«Управленческий пакет (P&L + CF)», отчётная дата выбирается в окне.",
        ]),
        ("Цепочка запросов DuckDB", [
            "base → base_stocks → wb_costs → dayly_sales_agg — подготовка "
            "первичных витрин продаж, остатков и удержаний маркетплейса.",
            "margin → временная таблица month_margins_wb_long: разделы 1 "
            "(выручка и маржинальность) и 2 (юнит-экономика).",
            "opex → временная таблица opex: разделы 3-8, собираются из wb_costs "
            "и витрины pl_for_csv учётной системы по префиксам счетов "
            "610000 / 620000 / 420000 / 630000.",
            "cf → временная таблица cf: транзакционная витрина cf_to_csv, "
            "отфильтрованная по date_from.",
            "treasury → остатки денежных средств: обороты по банковским "
            "счетам (ba_balance_csv + справочник corporate_bankaccount) и "
            "баланс маркетплейса (wb_balance_for_csv) на отчётную дату.",
            "Лист «Юнит-экономика» использует те же витрины, плюс отдельно "
            "считает количество продаж и возвратов по таблице base и "
            "приводит удержания площадки к суммам без НДС.",
        ]),
        ("Сводная таблица по Cash Flow", [
            "Лист «Сводная CF» — обычная сводная таблица Excel на "
            "листе-источнике raw_cf. Никаких макросов и VBA в файле нет.",
            "Источник наполняется при формировании пакета, а сама сводная "
            "обновляется автоматически при открытии файла. Строки свёрнуты до "
            "третьего уровня; ниже раскрываются подстатьи, контрагенты и "
            "договоры.",
            "Поля сводной можно менять как угодно — на остальные листы пакета "
            "это не влияет: они считаются независимо, источник raw_cf "
            "используется только сводной.",
        ]),
        ("Расшифровки P&L вместо сводной по P&L", [
            "Прежняя сводная по P&L строилась на бухгалтерской витрине и не "
            "сходилась с управленческим отчётом о прибылях и убытках, поэтому "
            "из пакета убрана.",
            "Вместо неё в P&L появилась колонка «НОТА»: стрелка напротив "
            "строки ведёт на лист «Расшифровки P&L», где та же сумма "
            "разложена на статью, контрагента и договор — и обе цифры "
            "считаются из одной витрины.",
        ]),
        ("Группировка колонок", [
            "Месяцы сгруппированы по годам средствами Excel. Завершённые годы "
            "свёрнуты при открытии файла, их итоговые колонки «Итого <год>» "
            "остаются видимыми; текущий год раскрыт помесячно. Развернуть год "
            "можно кнопкой «+» над таблицей.",
            "Колонка «Итого <год>» — сумма месяцев этого года, «Итого за "
            "период» — сумма годовых итогов. Остатки денежных средств в Cash "
            "Flow не суммируются: в колонке года показано состояние на начало "
            "и конец этого года.",
        ]),
        ("Формулы в файле", [
            "Все итоги и промежуточные результаты записаны формулами Excel, "
            "а не числами: раздел раскрывается статьями, итог раздела — SUM по "
            "его строкам, промежуточные итоги — арифметика по строкам итогов.",
            "Проценты и средние значения также заданы формулами, поэтому "
            "пересчитываются автоматически и корректны в колонках YTD и ИТОГО.",
            "Свёрнутые колонки не влияют на расчёт: формулы суммируют месяцы "
            "независимо от того, скрыты они или нет.",
        ]),
        ("Контрольные проверки", [
            "Маржа продаж = Продажи без НДС минус себестоимость.",
            "Маржа после комиссии = Маржа продаж минус комиссия WB.",
            "Остаток ДС на конец = остаток на начало плюс чистый денежный поток; "
            "должен совпадать с фактическими остатками по счетам и балансом WB.",
            "Налог на прибыль в P&L равен статье 125200 в Cash Flow за тот же "
            "месяц.",
        ]),
        ("Ограничения", [
            "Налог на прибыль отражён кассовым методом, отложенные налоговые "
            "активы и обязательства не рассчитываются.",
            "Амортизация в периметре не начисляется, поэтому EBITDA совпадает с "
            "операционной прибылью до финансовых расходов.",
            "Блок 122000 в Cash Flow — зачёт удержаний маркетплейса, а не платёж "
            "с расчётного счёта.",
        ]),
    ]
...
def build_management_pack(date_from, start_year=DEFAULT_START_YEAR, out_path=None):
    ...
    # ------------------------------------------------------------- excel
    # книга собирается поверх скелета со сводными; если его нет —
    ...
    build_notes(ws_nnotes, "ПОЯСНЕНИЯ К РАСШИФРОВКАМ P&L",
                "Как устроены ноты, три уровня детализации и сверка с P&L",
                NOTES_PL_SHEET)
    ...
    build_notes(ws_src, "ИСТОЧНИКИ ДАННЫХ И РЕГЛАМЕНТ",
                "Витрины, запросы и порядок обновления пакета",
                notes_sources(date_from, start_year, months))
    ...
    contents = [
        ...
        ("Сводная CF", "Сводная таблица Excel по движению денежных средств: "
                       "разрез по видам деятельности, статьям, контрагентам "
                       "и договорам"),
        ("Пояснения P&L", "Методика расчёта каждого раздела и промежуточного итога"),
    ...
    # открывает книгу в режиме группового редактирования и сводные
    # таблицы в нём не работают («Cannot edit a PivotTable in group edit mode»).
    for sheet in wb.worksheets:
        for view in sheet.views.sheetView:
            view.tabSelected = False
    wb.active = 0

    # порядок листов: отчёты, сводные, пояснения, сырые данные
    order = [TOC_SHEET_NAME, "P&L", NOTES_SHEET_NAME, "Юнит-экономика",
             "Cash Flow", "Остатки ДС", "Сводная CF",
             "Пояснения P&L", "Пояснения Расшифровки",
             "Пояснения Юнит-экономика", "Пояснения Cash Flow",
             "Источники данных", "raw_cf"]
