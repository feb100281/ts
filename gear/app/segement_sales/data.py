...
def get_tree_data(
    ...
    with get_duckdb_conn_with_opt() as con:
        ...
        df = con.execute(
            ...
            ELSE PRINTF('%,.2f',ROUND(revenue_by_lv3/100/1_000,2)::double)||'K' end,
            ' 📦',
            CASE 
            when qty_by_lv3  > 1_000 then PRINTF('%,.2f',ROUND(qty_by_lv3/1_000,1)::double)||'K'
            ELSE PRINTF('%,d',ROUND(qty_by_lv3,0)::bigint) end
            ) as lv3_label,
            ...
            ELSE PRINTF('%,.2f',ROUND(revenue_by_items/100/1_000,2)::double)||'K' end,
            ' 📦',
            CASE 
            when qty_by_items  > 1_000 then PRINTF('%,.2f',ROUND(qty_by_items/1_000,1)::double)||'K'
            ELSE PRINTF('%,d',ROUND(qty_by_items,0)::bigint) end
            ) as item_label
