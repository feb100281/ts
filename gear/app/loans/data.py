...
def get_loans_snapshot(
    ...
    query = """
        ...
        SELECT
            ...
            CASE
                WHEN af.fn_id = 6 THEN 'Мы должны'
                WHEN af.fn_id = 9 THEN 'Нам должны'
                ELSE 'Направление не определено'
            END AS loan_direction_label,
            af.fn_id AS accrual_function_id,
            COALESCE(
                files.documents_count,
                0
            ) AS documents_count,
        ...
        LEFT JOIN LATERAL (
            ...
            WHERE aa.contract_id = ls.contract_id
        ) af ON TRUE
