...
def register_loans_callbacks(app):
    ...
    def update_dashboard(
        ...
        direction = filtered.get(
            "loan_direction",
            pd.Series("unknown", index=filtered.index),
        ).fillna("unknown").astype(str)

        borrowed = filtered[
            direction.eq("borrowed")
        ].copy()
        issued = filtered[
            direction.eq("issued")
        ].copy()
        unknown = filtered[
            ~direction.isin(["borrowed", "issued"])
        ].copy()

        borrowed_kpis = calculate_kpis(borrowed)
        issued_kpis = calculate_kpis(issued)

        borrowed_ids = (
            borrowed["contract_id"]
            .dropna()
            .astype(int)
            .unique()
            .tolist()
        ...
        for loan_direction, ids in (
            ("borrowed", borrowed_ids),
            ("issued", issued_ids),
        ):
            if not ids:
                continue

            direction_dynamics = get_portfolio_dynamics(
                date_from,
                date_to,
                ids,
            )
            if not direction_dynamics.empty:
                direction_dynamics["loan_direction"] = loan_direction
                dynamics_parts.append(direction_dynamics)

            direction_interest = get_interest_flow(
                date_from,
                date_to,
                ids,
            )
            if not direction_interest.empty:
                direction_interest["loan_direction"] = loan_direction
                interest_parts.append(direction_interest)

        dynamics_df = (
            pd.concat(dynamics_parts, ignore_index=True)
            if dynamics_parts
            else pd.DataFrame()
        )
        interest_df = (
            pd.concat(interest_parts, ignore_index=True)
            if interest_parts
            else pd.DataFrame()
        )
        ...
        return (
            ...
            _pair(
                ...
                issued_kpis["total_debt"],
                _format_money,
            ),
            _pair(
                borrowed_kpis["principal_debt"],
                issued_kpis["principal_debt"],
                _format_money,
            ),
            _pair(
            ...
            build_debt_dynamics_chart(dynamics_df),
            _render_insights(build_debt_dynamics_insight(
                (
                    dynamics_df[
                        dynamics_df["loan_direction"].eq("borrowed")
                    ]
            ...
            build_counterparty_debt_chart(filtered),
            _render_insights(build_counterparty_debt_insight(borrowed)),
            
            build_maturity_chart(filtered),
            _render_insights(build_maturity_insight(borrowed)),
            
            build_interest_flow_chart(interest_df),
            _render_insights(build_interest_flow_insight(
                (
                    interest_df[
                        interest_df["loan_direction"].eq("borrowed")
                    ]
    ...
    def select_loan(
        ...
        return {
            ...
            "loan_direction": row.get(
                "loan_direction"
            ),

            "loan_direction_label": row.get(
