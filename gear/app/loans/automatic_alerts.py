...
# from .management_common import get_document_counts, get_management_snapshot
...
# def build_alerts(snapshot: pd.DataFrame, report_date: str) -> list[dict]:
#     report = _as_date(report_date) or date.today()
#     if snapshot.empty:
#         return []

#     work = snapshot.copy()
#     ids = [int(x) for x in work["contract_id"].dropna().tolist()]
#     docs = get_document_counts(ids)

#     if not docs.empty:
#         work = work.merge(docs, how="left", on="contract_id", suffixes=("", "_actual"))
#         if "documents_count_actual" in work.columns:
#             work["documents_count"] = work["documents_count_actual"].fillna(
#                 work.get("documents_count", 0)
#             )
...
#         cp = row.get("counterparty_name") or "Контрагент не указан"
#         number = row.get("contract_number") or "б/н"
#         maturity = _as_date(row.get("repayment_date"))
#         docs_count = int(row.get("documents_count") or 0)
#         rate = float(row.get("rate") or 0)

#         if maturity:
#             days = (maturity - report).days
#             if days < 0:
#                 alerts.append({
#                     "severity": "critical",
#                     "title": f"Просрочено погашение · {cp}",
#                     "text": f"Договор № {number}: срок прошёл {abs(days)} дн. назад, долг {debt:,.2f}".replace(",", " "),
#                 })
#             elif days <= 7:
#                 alerts.append({
#                     "severity": "critical",
#                     "title": f"Погашение через {days} дн. · {cp}",
#                     "text": f"Договор № {number}: общий долг {debt:,.2f}".replace(",", " "),
#                 })
#             elif days <= 30:
#                 alerts.append({
#                     "severity": "warning",
#                     "title": f"Срок ≤ 30 дней · {cp}",
#                     "text": f"Договор № {number}: до погашения {days} дн., долг {debt:,.2f}".replace(",", " "),
#                 })
#         else:
#             alerts.append({
#                 "severity": "warning",
#                 "title": f"Не указана дата погашения · {cp}",
#                 "text": f"Договор № {number}: невозможно построить полный план возврата тела займа.",
#             })

#         if docs_count == 0:
#             alerts.append({
#                 "severity": "warning",
#                 "title": f"Нет документов · {cp}",
#                 "text": f"По договору № {number} не найден ни один загруженный файл.",
#             })

#         if rate <= 0:
#             alerts.append({
#                 "severity": "info",
#                 "title": f"Ставка 0% / не задана · {cp}",
#                 "text": f"Проверьте ставку по договору № {number}.",
#             })
...
#             "bg": COLORS.get("light_red", "#FEF2F2"),
#             "fg": COLORS.get("red", "#B91C1C"),
#         },
#         "warning": {
#             "icon": "solar:danger-triangle-linear",
#             "bg": COLORS.get("light_orange", "#FFF7ED"),
#             "fg": COLORS.get("orange", "#C2410C"),
#         },
#         "info": {
#             "icon": "solar:info-circle-linear",
#             "bg": COLORS.get("light_gray", "#F3F4F6"),
#             "fg": COLORS.get("blue", "#2563EB"),
#         },
#     }[severity]
...
#         },
#         children=[
#             DashIconify(icon=style["icon"], width=17, color=style["fg"]),
#             html.Div([
#                 html.Div(alert["title"], style={"fontSize": "11px", "fontWeight": 700}),
#                 html.Div(alert["text"], style={"fontSize": "10px", "color": COLORS["muted"], "marginTop": "2px"}),
#             ]),
#         ],
#     )


# def build_automatic_alerts_panel():
#     return html.Div(
#         style={
...
#                 },
#                 children=[
#                     dmc.Text("Требует внимания", fw=700, size="sm"),
#                     html.Span(
#                         id=ALERTS_COUNT_ID,
#                         children="0",
...
#             html.Div(
#                 id=ALERTS_LIST_ID,
#                 children=dmc.Text("Нет предупреждений", size="xs", c=COLORS["muted"]),
...
# def register_automatic_alerts_callbacks(app):
#     @app.callback(
#         Output(ALERTS_LIST_ID, "children"),
#         Output(ALERTS_COUNT_ID, "children"),
#         Input(DATA_SIGNAL_ID, "data"),
#         Input(REPORT_DATE_ID, "value"),
#     )
#     def update_alerts(_signal, report_date):
#         if not report_date:
#             return dmc.Text("Нет данных", size="xs", c=COLORS["muted"]), "0"

#         snapshot = get_management_snapshot(str(report_date)[:10])
#         alerts = build_alerts(snapshot, str(report_date)[:10])

#         if not alerts:
#             return (
#                 html.Div(
#                     "Критичных событий не найдено",
#                     style={"padding": "16px", "fontSize": "11px", "color": COLORS["muted"]},
#                 ),
#                 "0",
#             )

#         return [_alert_row(x) for x in alerts], str(len(alerts))
