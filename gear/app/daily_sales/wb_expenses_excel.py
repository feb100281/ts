...
def report_menu_group(title, items, disabled=False):
    ...
    Дарья (после того как в меню появился второй отчёт, "Штрафы /
    Логистика"): меню уже не читается, хочется выделять отчёты
    нагляднее, "тем более у нас будет много ещё всяких отчётов там".
    Вложенное меню -- стандартный паттерн Mantine (Menu внутри Menu,
...
def export_menu_dropdown_items():
    ...
    Сам отчёт -- report_menu_group с одним форматом внутри; название
    формата ("Расшифровка, Excel") -- та же формулировка, что и у
    остальных отчётов в меню, чтобы при добавлении новых форматов
    (например, PDF-версии) не пришлось переименовывать этот пункт.
    """
    return [
