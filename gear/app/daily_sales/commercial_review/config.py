...
#: Насколько старым может быть срез заказов FBS, чтобы ему ещё
#: можно было доверять. Заказы снимаются из API WB регулярно;
#: если снимок старше порога, "просрочка" в нём не значит
#: реальную просрочку -- она значит, что статусы просто не
#: обновлялись, и писать про неё как про проблему нельзя.
FBS_STALE_HOURS = 30

#: Товар моложе этого возраста на складе не считается
#: залежавшимся, даже если ещё ни разу не продавался: ему
#: могло не хватить времени дойти до покупателя.
STOCK_NEW_ARRIVAL_DAYS = 30

#: Доля запаса, вошедшая в раздел риска, ниже которой отдельно
#: упоминать "новый товар" не имеет смысла -- это статистический
#: шум, а не сигнал.
STOCK_NEW_ARRIVAL_NOTE_PCT = 3.0

#: Сколько последних закрытых недель показываем в разделе
#: расходов WB.
WB_EXPENSES_WEEKS = 5

#: Неделя считается всплеском расходов, если она настолько
#: выше среднего по остальным неделям окна.
WB_EXPENSES_SPIKE_RATIO = 1.25

#: Статью расходов с таким весом в общей сумме за окно
#: не подсвечиваем как всплеск, даже если формально сработал
#: порог выше, -- это шум на маленьких числах, а не сигнал.
WB_EXPENSES_SPIKE_MIN_SHARE_PCT = 4.0
