...
def cost_money(value, dash="—") -> str:
    """
    Сумма расхода для страниц, где буквально всё -- статья
    расходов (например, "Анализ расходов WB"). money() кладёт
    отрицательное в скобки -- это нужно там, где на одной
    странице соседствуют доходы и расходы (например,
    "Финансовый результат"), но здесь каждая цифра и так расход,
    и скобки на каждой строке только шумят.

    Обычный расход (число отрицательное) -- чистая сумма без
    скобок и минуса. Редкий зачёт в нашу пользу (сумма вышла
    положительной, например вернули штраф) -- с явным плюсом,
    чтобы отличаться от обычных расходов.
    """
    v = num(value)
    if v is None:
        return dash
...
def cost_money_exact(value, dash="—") -> str:
    """То же самое, что cost_money(), но без сокращения до млн/млрд -- для таблиц."""
    v = num(value)
    if v is None:
        return dash
