...
def _add_sheet_summary_cards(
    ...
    # На листе FBS отдельная карточка
    # "Количество" не нужна:
    # там показываем именно "Остаток FBS".
    if ws.title != "FBS":
        cards.append(
            (
                "Количество",
...
def _build_summary_sheet(
    ...
    # --------------------------------------------------------------
    # Рабочая ширина сводки:
    # 3 карточки × 2 колонки = 6 колонок
    # --------------------------------------------------------------
    last_col = 6
    ...
    for idx, (
        ...
        # Каждый новый ряд карточек начинается через 4 строки
        r1 = start_row + (
            idx // 3
        ) * 4
    ...
    # Ширина колонок
    #
    # Каждая карточка занимает две колонки.
    # --------------------------------------------------------------
    for col_letter in [
        "A",
        "B",
...
def _build_fbs_sheet(
    wb,
    df,
    report_date,
):
    """
    Отдельный лист товаров с остатком FBS > 0.

    Никаких отдельных FBS-цен нет.
    ...
    Но все итоговые стоимости на этом листе
    считаются только от количества FBS.
    """

    if (
        df is None
        or df.empty
        or "Остаток FBS" not in df.columns
    ):
        return None
    ...
    # ============================================================
    # НА ЛИСТЕ FBS ИТОГО = ТОЛЬКО FBS
    #
    # Это нужно, чтобы стандартная summary-карточка
    # "Количество" показывала именно FBS.
    # ============================================================

    fbs_df["Итого количество"] = (
        fbs_df["Остаток FBS"]
    )
    ...
    # РОЗНИЧНАЯ СТОИМОСТЬ
    #
    # Та же самая розничная цена товара.
    # ============================================================
    ...
    if (
        ...
        fbs_df["Остатки в розничных ценах"] = (
            fbs_df["Остаток FBS"]
            * retail_price
        )
    ...
    if "Бух. с/с за ед." in fbs_df.columns:
        buh_cost = (
            pd.to_numeric(
                fbs_df["Бух. с/с за ед."],
                errors="coerce",
            )
            .fillna(0)
        )

        fbs_df["Бух. с/с всего"] = (
            fbs_df["Остаток FBS"]
            * buh_cost
        )
    ...
    if "Упр. с/с за ед." in fbs_df.columns:
        man_cost = (
            pd.to_numeric(
                fbs_df["Упр. с/с за ед."],
                errors="coerce",
            )
            .fillna(0)
        )

        fbs_df["Упр. с/с всего"] = (
            fbs_df["Остаток FBS"]
            * man_cost
        )
    ...
    return _write_dataframe_sheet(
        ...
        title="Остатки FBS",
        subtitle=(
            "Физические остатки на складе продавца: "
            "количество, розничная цена, "
            "себестоимость и стоимость остатка"
        ),
    )
        ...
        sheets_info.append(
            {
                "name": fbs_name,
                "description": (
                    "Отдельная детализация товаров "
                    "на складе продавца FBS"
                ),
            }
        )
