...
def get_incident_potential_prices(nm_ids, report_date):
    ...
    - если товар продан менее 10 раз за 365 дней, Ц = среднее
      ...
      ("Категория" в этом приложении), за тот же период. Это
      среднее считается по ВСЕМУ каталогу продавца (все nm_id
      этого Предмета, а не только те, что переданы в nm_ids) —
      иначе для Предметов, где в конкретном происшествии
      пострадал только один товар, усреднять было бы не с чем.
    ...
    with get_duckdb_conn_with_opt() as con:
        df = con.execute(
            ...
            */
            ...
            /*
            ============================================================
            КАТЕГОРИИ, ПО КОТОРЫМ ВООБЩЕ НУЖНО СРЕДНЕЕ (фолбэк для
            товаров с <10 продаж) - это Предметы запрошенных nm_ids.
            ============================================================
            */
            needed_subjects AS (
                SELECT DISTINCT
            ...
            /*
            ============================================================
            ВСЕ nm_id ПО ВСЕМУ КАТАЛОГУ (не только запрошенные!),
            относящиеся к нужным Предметам - иначе среднее по
            категории считалось бы только по товарам ИЗ ОДНОГО
            происшествия, а не по всему ассортименту этого Предмета.
            ============================================================
            */
            subject_universe AS (
                SELECT
                ...
                WHERE
                    COALESCE(
                        ...
                        SELECT subject_name FROM needed_subjects
                    )
            ),
            ...
            subject_price_rows AS (
                ...
                JOIN subject_universe su
                    ON su.nm_id = pr.nm_id
            ),
