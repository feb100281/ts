...
def preview(cond, anchor_date):
    ...
    for m in iter_months(start, finish):
        ...
        part = (monthly_amount * days_used / days_in_month) if days_in_month else Decimal("0")
        part = q2(part)
        ...
        rows.append({
            ...
            "comment": f"(БАП+ЭП)*площадь = ({bap}+{ep})*{area} • {int(days_used)}/{int(days_in_month)} дней",
        })
