# reporting/excel/styles/overhead_expenses.py
...
def _draw_header(ws, date_to=None):
    draw_back_to_pl_button(ws, cell="A1", target_sheet="PL")
...
def _style_name_cell(cell, level, is_total_row, row_num):
    ...
    cell.alignment = Alignment(horizontal="left", vertical="center", indent=4)
...
def _display_name(display_name, code):
...
def _apply_row_outline(ws, row_positions, row_meta):
    ...
    # level 2 под level 1
    parents_lvl1 = meta[meta["level"] == 1]["row_key"].tolist()
    for parent_key in parents_lvl1:
        child_rows = meta[
            (meta["level"] == 2) &
    ...
    # level 3 под level 2 — сразу скрыты
    parents_lvl2 = meta[meta["level"] == 2]["row_key"].tolist()
    for parent_key in parents_lvl2:
        cp_rows = meta[
            (meta["level"] == 3) &
            (meta["parent_row"] == parent_key)
        ]["row_key"].tolist()

        excel_rows = [row_positions[r] for r in cp_rows if r in row_positions]
        if not excel_rows:
            continue
        ...
        # чтобы Excel показал плюсик на строке статьи
        parent_excel_row = row_positions.get(parent_key)
        if parent_excel_row:
            ws.row_dimensions[parent_excel_row + 1].collapsed = True
...
def _need_blank_after_row(current_key, next_key, row_meta):
    ...
    next_level = int(row_meta.loc[next_key, "level"])

    # после группы пустую строку не рисуем
    if current_level == 1:
        return False

    # статья -> её контрагент
    if current_level == 2 and next_level == 3 and row_meta.loc[next_key, "parent_row"] == current_key:
        return False

    # статья -> следующая статья/группа
    if current_level == 2:
    ...
    # контрагент -> следующий контрагент той же статьи
    if current_level == 3:
        current_parent = row_meta.loc[current_key, "parent_row"]
        next_parent = row_meta.loc[next_key, "parent_row"]
        return current_parent != next_parent
...
def style_overhead_expenses_sheet(ws, payload, date_to=None):
    _clear_sheet(ws)
    ...
    for idx, row_key in enumerate(row_keys):
        ...
        if is_total_row:
            ...
            display_name = _display_name(raw_name, code)
    ...
    draw_nav_link_row(
        ...
        target_sheet="1.4_drill_down",
        target_cell="A1",
    )

    total_cols = 1 + len(visual_columns)

    current_year = pd.to_datetime(date_to).year if date_to is not None else None
