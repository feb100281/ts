...
#     План > 0 (доход): положительная дельта = зеленый, отрицательная = красный
#     План < 0 (расход): отрицательная дельта = зеленый, положительная = красный
#     """
#     try:
...
#     """Расчет YTD значений для строки (рубли и проценты)"""
#     ytd_rub = {}
#     ytd_pct = {}
#     running_rub = 0
#     running_plan = 0
...
# def _hide_fact_and_delta_columns(ws, month_layout, total_fact_col, total_delta_rub_col, total_delta_pct_col):
#     ws.sheet_properties.outlinePr.summaryBelow = False
#     ws.sheet_properties.outlinePr.summaryRight = True
#     ws.sheet_view.showOutlineSymbols = True
...
#     for col in (total_fact_col, total_delta_rub_col, total_delta_pct_col):
#         col_letter = get_column_letter(col)
#         ws.column_dimensions[col_letter].hidden = True
#         ws.column_dimensions[col_letter].outline_level = 1
...
#     # Получаем план для определения типа (доход/расход)
#     plan_total = row.get("plan_total", 0)
...
#     # Рассчитываем YTD для строки
#     ytd_rub, ytd_pct = _calculate_ytd_for_row(row["plan_months"], row["fact_months"], months)
...
#             # Для дочерних строк проценты не показываем
#             if is_child:
#                 cell = ws.cell(row=row_idx, column=col_idx, value=None)
#                 cell.fill = FILLS["none"]
#                 cell.border = BORDERS["none"]
...
#             value = _format_percent(delta_pct)
#             cell = ws.cell(row=row_idx, column=col_idx, value=value)
...
#     # Дельта % итого - для дочерних строк не показываем
#     if not is_child:
...
#         total_delta_pct_cell = ws.cell(row=row_idx, column=total_delta_pct_col, value=_format_percent(total_delta_pct_value))
...
#     else:
#         # Пустая ячейка для дочерней строки
...
#     # YTD руб и % итого (последний месяц)
#     if months:
#         last_month = months[-1]
#         total_ytd_rub_value = ytd_rub.get(last_month, 0)
#         total_ytd_pct_value = ytd_pct.get(last_month, 0)
#     else:
#         total_ytd_rub_value = 0
#         total_ytd_pct_value = 0
    
#     total_ytd_rub_cell = ws.cell(row=row_idx, column=total_ytd_rub_col, value=total_ytd_rub_value)
#     total_ytd_rub_cell.fill = _get_delta_fill(total_ytd_rub_value, FILLS["none"], total_plan_value)
#     total_ytd_rub_cell.font = _get_row_font(total_ytd_rub_value, is_child=is_child)
#     total_ytd_rub_cell.alignment = ALIGNMENTS["right"]
#     total_ytd_rub_cell.border = BORDERS["thin"]
#     total_ytd_rub_cell.number_format = FORMATS["money_int"]
    
#     # YTD % итого - для дочерних строк не показываем
#     if not is_child:
#         total_ytd_pct_cell = ws.cell(row=row_idx, column=total_ytd_pct_col, value=_format_percent(total_ytd_pct_value))
#         total_ytd_pct_cell.fill = _get_delta_fill(total_ytd_rub_value, FILLS["none"], total_plan_value)
#         total_ytd_pct_cell.font = _get_row_font(total_ytd_rub_value, is_child=is_child)
#         total_ytd_pct_cell.alignment = ALIGNMENTS["center"]
#         total_ytd_pct_cell.border = BORDERS["thin"]
#     else:
#         # Пустая ячейка для дочерней строки
#         empty_cell = ws.cell(row=row_idx, column=total_ytd_pct_col, value=None)
#         empty_cell.fill = FILLS["none"]
#         empty_cell.border = BORDERS["none"]
...
#             total_ytd_rub_col=total_ytd_rub_col,
#             total_ytd_pct_col=total_ytd_pct_col,
#             is_child=False,
#         )
...
#     # Рассчитываем YTD накопленным итогом для total строки
#     running_ytd_rub = 0
#     running_ytd_plan = 0
#     ytd_rub_by_month = {}
#     ytd_pct_by_month = {}
...
#             delta_pct = ytd_pct_by_month.get(month, 0)
#             cell = ws.cell(row=row_idx, column=col_idx, value=_format_percent(delta_pct))
#             cell.fill = _get_delta_fill(delta_rub, FILLS["total"], plan_val)
#             cell.font = FONTS["total"]
#             cell.alignment = ALIGNMENTS["center"]
...
#     total_delta_pct_cell = ws.cell(row=row_idx, column=total_delta_pct_col, value=_format_percent(total_delta_pct_sum))
...
#     # YTD руб и % итого (последнее накопленное значение)
#     if months:
#         last_month = months[-1]
#         total_ytd_rub_sum = ytd_rub_by_month.get(last_month, 0)
#         total_ytd_pct_sum = ytd_pct_by_month.get(last_month, 0)
#     else:
#         total_ytd_rub_sum = 0
#         total_ytd_pct_sum = 0
    
#     total_ytd_rub_cell = ws.cell(row=row_idx, column=total_ytd_rub_col, value=total_ytd_rub_sum)
#     total_ytd_rub_cell.fill = _get_delta_fill(total_ytd_rub_sum, FILLS["total"], total_plan_sum)
#     total_ytd_rub_cell.font = FONTS["negative_total"] if total_ytd_rub_sum < 0 else FONTS["total"]
#     total_ytd_rub_cell.alignment = ALIGNMENTS["right"]
#     total_ytd_rub_cell.border = BORDERS["bottom_medium"]
#     total_ytd_rub_cell.number_format = FORMATS["money_int"]
    
#     total_ytd_pct_cell = ws.cell(row=row_idx, column=total_ytd_pct_col, value=_format_percent(total_ytd_pct_sum))
#     total_ytd_pct_cell.fill = _get_delta_fill(total_ytd_rub_sum, FILLS["total"], total_plan_sum)
#     total_ytd_pct_cell.font = FONTS["total"]
#     total_ytd_pct_cell.alignment = ALIGNMENTS["center"]
#     total_ytd_pct_cell.border = BORDERS["bottom_medium"]

#     _hide_fact_and_delta_columns(ws, month_layout, total_fact_col, total_delta_rub_col, total_delta_pct_col)
...
def _draw_detail_value_row(
    ...
    if months:
        ...
        total_ytd_rub_value = ytd_rub.get(last_month, 0)
        total_ytd_pct_value = ytd_pct.get(last_month, 0)
        last_plan_val = row["plan_months"].get(last_month, 0)
    else:
        total_ytd_rub_value = 0
        total_ytd_pct_value = 0
        last_plan_val = 0
    ...
    total_ytd_rub_cell = ws.cell(row=row_idx, column=total_ytd_rub_col, value=total_ytd_rub_value)
    total_ytd_rub_cell.fill = _get_delta_fill(total_ytd_rub_value, FILLS["none"], last_plan_val)
    total_ytd_rub_cell.font = _get_row_font(total_ytd_rub_value, is_child=is_child)
    total_ytd_rub_cell.alignment = ALIGNMENTS["right"]
    total_ytd_rub_cell.border = BORDERS["thin"]
    total_ytd_rub_cell.number_format = FORMATS["money_int"]
    ...
    if not is_child:
        total_ytd_pct_cell = ws.cell(row=row_idx, column=total_ytd_pct_col, value=_format_percent(total_ytd_pct_value))
        total_ytd_pct_cell.fill = _get_delta_fill(total_ytd_rub_value, FILLS["none"], last_plan_val)
        total_ytd_pct_cell.font = _get_row_font(total_ytd_rub_value, is_child=is_child)
        total_ytd_pct_cell.alignment = ALIGNMENTS["center"]
        total_ytd_pct_cell.border = BORDERS["thin"]
    else:
...
def build_gl_detail_sheet(wb, detail):
    ...
    if months:
        ...
        total_ytd_rub_sum = ytd_rub_by_month.get(last_month, 0)
        total_ytd_pct_sum = ytd_pct_by_month.get(last_month, 0)
        last_plan_val = detail["total_plan_by_month"].get(last_month, 0)
    else:
        total_ytd_rub_sum = 0
        total_ytd_pct_sum = 0
        last_plan_val = 0
    ...
    total_ytd_rub_cell = ws.cell(row=row_idx, column=total_ytd_rub_col, value=total_ytd_rub_sum)
    total_ytd_rub_cell.fill = _get_delta_fill(total_ytd_rub_sum, FILLS["total"], last_plan_val)
    total_ytd_rub_cell.font = FONTS["negative_total"] if total_ytd_rub_sum < 0 else FONTS["total"]
    total_ytd_rub_cell.alignment = ALIGNMENTS["right"]
    total_ytd_rub_cell.border = BORDERS["bottom_medium"]
    total_ytd_rub_cell.number_format = FORMATS["money_int"]
    ...
    total_ytd_pct_cell = ws.cell(row=row_idx, column=total_ytd_pct_col, value=_format_percent(total_ytd_pct_sum))
    total_ytd_pct_cell.fill = _get_delta_fill(total_ytd_rub_sum, FILLS["total"], last_plan_val)
    total_ytd_pct_cell.font = FONTS["total"]
    total_ytd_pct_cell.alignment = ALIGNMENTS["center"]
    total_ytd_pct_cell.border = BORDERS["bottom_medium"]
