...
from utils.vat_check.vat_exceptions import VAT_EXCEPTIONS_NM_IDS
from utils.vat_check.vat_rules import CHILDREN_10_VAT_TNVED_RULES, REDUCED_VAT_RATE, is_children_by_gender
...
def validate_vat_rate_simple(
    ...
    current_vat_rate: float | int | None,
    nm_id: Any = None,
    characteristics: list[dict[str, Any]] | None = None,  # ← ДОБАВИТЬ
) -> dict[str, Any]:
    """
    Проверяем:
    - если код ТН ВЭД льготный ИЛИ характеристика "Пол" детский -> у товара должна быть ставка 10%.
    Исключенные nm_id не считаются ошибкой.
    """
    normalized_nm_id = _normalize_nm_id(nm_id)
    if normalized_nm_id in VAT_EXCEPTIONS_NM_IDS:
    ...
    # Проверяем по полу
    is_children_gender = is_children_by_gender(characteristics) if characteristics else False
    
    # Товар должен быть льготным если:
    # - ТН ВЭД детский ИЛИ
    # - Пол детский
    should_be_reduced = matched_rule is not None or is_children_gender
    ...
    if should_be_reduced:
        ...
        if not is_correct_rate:
            # Определяем причину, почему должен быть льготным
            if matched_rule and is_children_gender:
                reason = f"ТН ВЭД ({matched_rule.get('name')}) и Пол детский"
            elif matched_rule:
                reason = f"ТН ВЭД ({matched_rule.get('name')})"
            else:
                reason = "Пол детский"
            
            if current_vat_rate is None:
                message = f"Детский товар ({reason}), но ставка основная (должно быть 10%)"
            else:
                actual_rate = float(current_vat_rate)
                message = (
                    f"Детский товар ({reason}), но указана основная ставка "
                    f"{int(actual_rate)}% вместо льготной 10%"
                )

            return {
                "has_issue": True,
                "issue_type": "children_product_with_wrong_rate",
                "severity": "error",
                "message_short": message,
                "matched_rule_name": matched_rule.get("name") if matched_rule else "Пол детский",
                "normalized_tnved_code": normalized_code,
                "normalized_nm_id": normalized_nm_id,
                "expected_rate": REDUCED_VAT_RATE,
                "current_rate": current_vat_rate,
...
def get_vat_validation_report(
    ...
    - vat_rate
    - tnved_code
    - characteristics (JSON или список характеристик)
    - sales_YYYY
    - quantity_YYYY
    - last_sale_date
    """
    ...
    for _, row in df.iterrows():
        ...
        # Проверяем, должен ли товар быть льготным
        matched_rule = _match_tnved_rule(row.get("tnved_code"), CHILDREN_10_VAT_TNVED_RULES)
        is_children_gender = is_children_by_gender(characteristics) if characteristics else False
        
        should_be_reduced = matched_rule is not None or is_children_gender
        
        if not should_be_reduced:
            continue
