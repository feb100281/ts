# budget/reporting/pdf/charts/weekly_trends_chart.py
...
def build_weekly_trends_chart_base64(weekly_trends: list) -> str | None:
    """
    График недельной динамики чистой выручки.
    """
    ...
    y = df["net_amount"].fillna(0).astype(float).tolist()
    if not y:
        return None
    ...
    ax.set_yticks(ticks)
    ax.set_yticklabels(
        [_fmt_mln(t) if t >= 0 else "" for t in ticks],
        fontsize=8,
        color=COLOR_MUTED,
    )
    ...
    ax.set_title(
        "Недельная динамика чистой выручки",
        loc="left",
        fontsize=12,
        fontweight="bold",
        color=COLOR_TEXT,
